...
So we're left with `r sum(rw.7k)` sequences 
in `r nrow(rw.7k)` samples 
and `r ncol(rw.7k[, colSums(rw.7k)>0])` OTUs. 
...
```{r top5MedianAbundance, results='asis'}
...
```{r phylumBarplot,dpi=100,fig.height=4.5,fig.width=4.5, eval=FALSE}
...
What percentage of observations are represented here? `r sum(colMeans(act.table.10))*100`%
...
```{r longPhylumBarplots, fig.height=6, fig.width=15}
...
Yes - both sexes share, on average, more with their own phones than with everyone else's. Men share about `r diff.m`% percent more taxa with their own than with other phones, while women share `r diff.f`% more than they share with other people's phones. 

Here's the figure that goes in the manuscript. 

```{r makeLongPersonToPhoneBarplot, fig.width=5, fig.height=4, tidy=FALSE}
...
It looks like `r the.1percent` of the OTUs represent more than 0.1% of the dataset. So the next move is to see how many of these (for each person) are shared between index and phone, and also between index and thumb. 

```{r eachPoint1percent, results='asis'}
